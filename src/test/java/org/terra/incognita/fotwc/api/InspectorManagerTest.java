            ...
            httpsServer.setHttpsConfigurator(new HttpsConfigurator(sslContext) {
                public void configure(HttpsParameters params) {
                    try {
                        ...
                        log.error("Failed to create HTTPS server");
                    }
                }
            });
            log.atDebug().log("HTTPS Server started at {}:{}","localhost", 8443);
        } catch (IOException | KeyStoreException | NoSuchAlgorithmException | UnrecoverableKeyException | CertificateException | KeyManagementException ioe) {
